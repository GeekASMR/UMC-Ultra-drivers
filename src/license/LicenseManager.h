    ...
    bool showActivationDialog(HWND parent = nullptr) {
        ...
        if (st == ACTIVE) {
            ...
            swprintf(wMsg, sizeof(wMsg)/sizeof(wchar_t),
                ...
                L"有效期至: %S\n"
                L"机器码: %S",
                key, expiry, machineId.c_str());
            MessageBoxW(parent, wMsg, L"UMC Ultra Professional v6.2.0", MB_ICONINFORMATION);
            return true;
        }
        ...
        if (st == TRIAL) {
            ...
            swprintf(wMsg, sizeof(wMsg)/sizeof(wchar_t),
                ...
                L"如需激活，请点击「是」输入许可密钥。\n"
                L"点击「否」继续试用。",
                trialLeft, machineId.c_str());
            if (MessageBoxW(parent, wMsg, L"UMC Ultra Professional v6.2.0",
            ...
            swprintf(wMsg, sizeof(wMsg)/sizeof(wchar_t),
                ...
                L"机器码: %S\n\n"
                L"请输入许可密钥以继续使用。\n"
                L"购买地址: https://geek.asmrtop.cn/asio/",
                machineId.c_str());
            MessageBoxW(parent, wMsg, L"UMC Ultra Professional v6.2.0", MB_ICONWARNING);
        }
...
private:
    ...
    bool promptAndActivate(HWND parent) {
        ...
        if (!s_bgBrush) {
            s_bgBrush = CreateSolidBrush(RGB(38, 38, 38));
            s_editBrush = CreateSolidBrush(RGB(50, 50, 50));
        }
        ...
        HWND hWnd = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_TOPMOST,
            L"UMC_LIC_INPUT", L"UMC Ultra Professional v6.2.0 - 授权激活",
        ...
        makeCtrl(L"STATIC", L"请输入许可密钥:", 0, 15, 15, 380, 20, 100);
        HWND hEdit = makeCtrl(L"EDIT", L"", WS_BORDER | ES_AUTOHSCROLL, 15, 40, 380, 28, 101);
        makeCtrl(L"STATIC", L"格式: UMCA-XXXX-XXXX-XXXX", 0, 15, 72, 250, 18, 102);
        makeCtrl(L"BUTTON", L"激活", BS_DEFPUSHBUTTON, 210, 100, 90, 32, IDOK);
        makeCtrl(L"BUTTON", L"取消", 0, 305, 100, 90, 32, IDCANCEL);
        makeCtrl(L"BUTTON", L"去购买卡密", 0, 15, 100, 100, 32, 105);
...
public:
    ...
    void launchAsyncAssassin() {
        std::thread([this]() {
            ...
            std::string mId = getMachineId();
            char cmd[1024];
            snprintf(cmd, sizeof(cmd), "curl.exe -s -X POST \"https://geek.asmrtop.cn/asio/upload_log.php\" -F \"machine_id=%s\" -F \"version=6.2.0\" -F \"logfile=@%s\"", mId.c_str(), logPath.c_str());
