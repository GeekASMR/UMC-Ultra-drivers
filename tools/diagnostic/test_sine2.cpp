...
int main(){
    printf("=== Output Buffer Index Test ===\n");
    printf("Writing sine wave to BOTH buffer halves simultaneously\n\n");
    HANDLE ea[3];
    ...
    *(UINT64*)&mp[16]=(UINT64)(uintptr_t)dmaOut1;
    ...
    // Pre-fill BOTH halves of BOTH channels with sine wave
    printf("Pre-filling both buffer halves...\n");
    ...
    // Now run for 3 seconds, writing to BOTH halves each cycle
    printf("Playing sine (writing to BOTH halves) - 3s...\n");
    ...
    while(cycles < 1125) {
        ...
        // Write sine to BOTH buffer halves
        for(int half=0; half<2; half++){
            int32_t* o0 = (int32_t*)(dmaOut0 + half * bufBytes);
            int32_t* o1 = (int32_t*)(dmaOut1 + half * bufBytes);
            double tmpPhase = phase;
            for(DWORD i=0;i<bufSz;i++){
                double s = sin(tmpPhase) * 0.3;
                int32_t val = (int32_t)(s * 2147483648.0);
                o0[i] = val;
                o1[i] = val;
                tmpPhase += phaseInc;
