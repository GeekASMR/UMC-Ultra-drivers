// 测试: Event 等待 + WAIT_FOR_BUFFER 触发内核复制
#include <windows.h>
#include <setupapi.h>
#include <stdio.h>
#include <math.h>
...
int main(){
    ...
    *(UINT64*)&mp[16]=(UINT64)(uintptr_t)dmaOut1;
    ...
    // Use a separate OVERLAPPED event for WFB calls
    HANDLE wfbEvt = CreateEventW(NULL, TRUE, FALSE, NULL);
    
    printf("Playing 3s with WAIT_FOR_BUFFER after write...\n\n");
    
    for(int c=0;c<1125;c++){
        // 1. Wait for event (kernel signals buffer ready)
        WaitForSingleObject(ea[0], 100);
        DWORD curCtr = *(volatile DWORD*)ctrl;
        if(curCtr == lastCtr) { c--; continue; }
        lastCtr = curCtr;
        
        // 2. Write sine to BOTH halves (brute force)
        ...
        // 3. Call WAIT_FOR_BUFFER to trigger kernel copy!
